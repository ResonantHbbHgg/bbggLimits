...
if 1 > 0 :
  ...
  histmhhRecoRe = ROOT.TH1D('MhhRecoRe', '', 30,0.,600.) 
  histmhhRecoSM = ROOT.TH1D('MhhRecoSM', '', 30,0.,600.) 
  histmhhRecoReA = ROOT.TH1D('MhhRecoSMA', '', 30,0.,600.) 

  histmhhBox = ROOT.TH1D('MhhBox', '', 30,0.,600.) 
  histmhhBoxA = ROOT.TH1D('MhhBoxA', '', 30,0.,600.) 
  histmhhL15 = ROOT.TH1D('MhhL15', '', 30,0.,600.) 
  histmhhL2p4 = ROOT.TH1D('MhhL2p4', '', 30,0.,600.) 
...
#########################################################
normBench = [ 49976.6016382, 50138.2521798, 49990.0468825, 572.0, 50041.0282539, 50038.5462286, 50001.0693263, 50000.3090638, 50045.3506862, 49992.1242267, 50024.7055638, 50006.2937198]
...
# loop in all events
countevent=0
for ifile in range(0,1) : # len(files)  
  ...
  for iev in range(0,nev) :    
    ...
    # make tree for test
    if 1 > 0 :
      Genmhh[0] = tree.genMHH
      GenHHCost[0] = tree.genCosth
      MT2[0] = tree.MT2
      ...
      if sumHAnalyticalBin.GetBinContent(bmhh,bcost) >0 : # to be done with all events
         ...
         weightSMA[0] = model.getScaleFactor(mhhcost,1.0, 1.0, effSumV0)/ model.getNormalization(1.0, 1.0,sumHAnalyticalBin) #normSManal
         sumWSMA+=weightSMA[0]
         # lambda scan
      ...
      histmhhRecoReA.Fill(MT2[0],weightSMA[0])
      histmhhRecoRe.Fill(MT2[0],weightSM[0])

      histmhhBoxA.Fill(MT2[0],weightL0[0])
      histmhhBench1.Fill(MT2[0],weight1[0])
      histmhhBench2.Fill(MT2[0],weight2[0])
      histmhhBench6.Fill(MT2[0],weight6[0])

      histmhhL15.Fill(MT2[0],weightL15[0])
      histmhhL2p4.Fill(MT2[0],weightL2p4[0])
      if ifile ==0 : 
        histmhhSM.Fill(Genmhh[0])
        histmhhRecoSM.Fill(MT2[0])
...
if 1 > 0 :
  ...
  histmhhRe.Draw("hist")
  leg.AddEntry(histmhhRe,"reweigted (from 300k SM events, hist)")
  #leg.AddEntry(histmhhRe,"reweigted (from 74k events, hist)")
  histmhhReA.Scale(1./histmhhReA.Integral())
  histmhhReA.SetLineWidth(2)
  histmhhReA.SetLineColor(2)
  histmhhReA.Draw("same,hist")
  leg.AddEntry(histmhhReA,"reweigted (from 300k SM, from analitical)")
  #leg.AddEntry(histmhhReA,"reweigted (from 74k events, from analitical)")
  histmhhSM.Scale(1./histmhhSM.Integral())
  histmhhSM.SetLineWidth(2)
  histmhhSM.SetLineColor(8)
  histmhhSM.Draw("same,hist")
  leg.AddEntry(histmhhSM,"SM (from 300k SM events)")
  #leg.AddEntry(histmhhSM,"SM (from ratio 2744 events)")
  leg.Draw("same")
  cs.SaveAs("SM_GenMhh_afterCuts_tatabb_HM.png") 
  #cs.SaveAs("SMtest_afterCuts_tatabb_LM.png")
  cs.Clear()
  leg.Clear()
  ###########################
  ...
  leg.AddEntry(histmhhRecoSM,"SM (from 300k SM events)")
  leg.Draw("same")
  cs.SaveAs("MT2_SM_Reco_afterCuts_tatabb.png") 
  cs.Clear()
  leg.Clear()
  ###########################
  """
  ...
  leg.AddEntry(histmhhBoxA,"#kappa_{#lambda} = 0 (from 300k SM events)")
  #leg.AddEntry(histmhhBoxA,"#kappa_{#lambda} = 0 (from 4503 events)")
  leg.AddEntry(histmhhBox,"reweigted (from analitical)")
  leg.Draw("same")
  #cs.SaveAs("Boxtest_afterCuts_tatabb_LM.png") 
  cs.SaveAs("Boxtest_afterCuts_tatabb_LM.png") 
  cs.Clear()
  leg.Clear()
  """
  ...
  histmhhBench1.SetAxisRange(0., 600.,"X")
  ...
  histmhhBench2.SetAxisRange(0., 600.,"X")
  ...
  histmhhBench6.SetAxisRange(0., 600.,"X")
  leg.AddEntry(histmhhBench6,"BM6")
  histmhhBench6.Draw("same,hist")
  leg.Draw("same")
  cs.SaveAs("Bench1_afterCuts_reco_tatabb.png") 
  cs.Clear()
  leg.Clear()
  leg.Clear()
  #################################################"
  ...
  histmhhRecoReA.Scale(1./histmhhRecoReA.Integral())
  histmhhRecoReA.SetLineWidth(2)
  histmhhRecoReA.SetLineColor(1)
  histmhhRecoReA.Draw("same,hist")
  leg.AddEntry(histmhhRecoReA,"SM")
  leg.Draw("same")
  cs.SaveAs("MT2_kl_afterCuts_reco_tatabb.png") 
print "done "
print "Sig ",countSig
