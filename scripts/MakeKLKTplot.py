...
leg.SetBorderSize(0)
leg.SetFillStyle(0)
#leg.SetHeader('pp#rightarrowHH#rightarrowb#bar{b}#gamma#gamma')
...
leg.AddEntry(band_1s_low, 'Expected #pm1#sigma', 'f')
leg.AddEntry(pt_sm, 'Standard Model', 'p')
pt_sm.Draw('p same')
if opt.unblind:
...
leg.AddEntry(band_2s_low, 'Expected #pm2#sigma', 'f')
leg.AddEntry(hframe, 'c_{g} = c_{2} = c_{2g} = 0', '')
...
for gr,gr in enumerate(myGraphs):
  if 'cen' in str(gr.GetName()):
    ...
    gr.GetZaxis().SetTitle('95% C.L. Expected Limit [fb]')
...
if opt.unblind == True:
  ...
  for gr,gr in enumerate(myGraphs):
    if 'obs' in str(gr.GetName()):
      ...
      gr.GetXaxis().SetTitle('#kappa_{#lambda}')
      gr.GetYaxis().SetTitle('#kappa_{t}')
      gr.GetZaxis().SetTitle('95% C.L. Observed Limit [fb]')
      gr.GetZaxis().SetRangeUser(0., 5.)
      cob.Update()
      SetAxisTextSizes(gr)
      cob.Update()
